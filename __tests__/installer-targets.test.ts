...
describe('Installer targets — partial-state idempotency', () => {
  ...
  it('claude: global lifecycle honors CLAUDE_CONFIG_DIR, leaving ~/.claude untouched', () => {
    const claude = getTarget('claude')!;
    ...
    try {
      ...
      const detected = claude.detect('global');
      expect(detected.alreadyConfigured).toBe(true);
      expect(path.resolve(detected.configPath!)).toBe(path.resolve(path.join(profile, '.claude.json')));

...
| Verilog / SystemVerilog | RTL designs (no framework) | module instantiation hierarchy (`top` → `uart_bridge` → `uart_rx`); `` `include `` header | — | ✅ **covered by extraction + the flow finder riding `instantiates` (#402)** — Tang Nano 9K eMMC reader (33 .v, 32 modules, 34 instantiations, all matching source), 2uart_to_1uart (11 .v, 11 instantiations), pulp-platform/axi (93 .sv, 177 modules/packages, 7 interfaces, 277 instantiations, 175 `include`/`import` edges, 2 files with `ifdef`-in-port-list parse errors that still index). A synthesis file's instantiation skips a same-named `sim/`/`tb/` stub. 🔬 frontier: `generate`-block conditional instantiation is unlabelled (no branch-guard rules), `` `define ``-named module types (`` `MY_PLL u (…) ``) do not resolve |

...
| Verilog / SystemVerilog | `.v`, `.vh`, `.sv`, `.svh` | Modules, packages, interfaces/modports and named instances; ports and signals with named/positional/wildcard port bindings, bit/part selects and always/assign blocks; lexical and generate scopes; read/write/control/event signal-access queries; optional build profiles (filelists/includes/defines, see [docs/hdl-profiles.md](docs/hdl-profiles.md)). Source graph only — no elaboration or timing; `codegraph hdl-semantic` computes parameters and widths through an installed [slang](docs/hdl-semantics.md) |

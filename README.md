...
| **20+ Languages** | TypeScript, JavaScript, ArkTS, Python, Go, Rust, Java, C#, VB.NET, PHP, Ruby, C, C++, CUDA, Objective-C, Metal, Swift, Kotlin, Scala, Dart, Lua, Luau, R, Nix, Erlang, CFML, COBOL, Solidity, Terraform/OpenTofu, Verilog/SystemVerilog, Svelte, Vue, Astro, Liquid, Pascal/Delphi |
...
| Verilog / SystemVerilog | `.v`, `.vh`, `.sv`, `.svh` | Full support (modules, interfaces, programs and packages as containers, functions and tasks, parameters, `` `define `` macros, typedefs, `import pkg::*` and `` `include `` edges, module-instantiation edges that carry callers/callees/impact and `codegraph_explore` flows through the design hierarchy; a synthesis file's instantiation skips a same-named `sim/`/`tb/` stub) |
